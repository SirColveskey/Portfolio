""" Boggle Simulator GUI Extension
By: Benjamin Cooper
FSUID: BJC12B
Date: 8/5/2016
Course: CIS4930
Assignment: Final
The purpose of this program is to create a GUI version of the word game Boggle(tm).
The program is event driven, beginning with the creation of a BoggleWindow.
The BoggleWindow calls a series of components to create a boggle board, a timer, and
a list and input box. At the end of the timer the current game is evaluated.
Games can be saved using the file menu. Games can be loaded from the same
menu or at the launch of the program. The files are saved in two contingent files
Boggle.save and Boggle.ids -- if either of these files are removed 
and one remains errors may occur. The program draws initial design inspiration from
the pegboard game example in class. The program is built using PyQt5. Enjoy.


BELOW IS THE HEADER FROM THE INCLUDED BOGGLE.PY
...
Course: CIS4930
Assignment: 1
The purpose of this program is to create a simulator for the word game Boggle(tm).
The program is executed by running the function play_boggle(), at which point it
will print out a standard boggle board and allow the user an unlimited amount of
time to produce possible words from the board. The user will conclude their inputs
by providing the input 'X'. After this the program will proceed to evaluate their
inputted words for A) Validity according to the rule set, B) Presence on the board
and C) Being a valid word in en_US dictionary (courtesy of PyEnchant). The score
is then provided."""
